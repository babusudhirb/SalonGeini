...
function walkInLine(item, index) {
  ...
  return `<div class="walkin-line" data-walkin-line="${index}"><span class="walkin-service-detail"><strong>${treatment.name}</strong><small>${treatment.category || 'Salon service'}</small></span><label>Staff<select data-walkin-item="staffId" data-walkin-index="${index}">${team.map(member => `<option value="${member.id}" ${Number(item.staffId) === member.id ? 'selected' : ''}>${member.name}</option>`).join('')}</select></label><label>Qty<input type="number" min="1" value="${item.quantity}" data-walkin-item="quantity" data-walkin-index="${index}"></label><label>Price<input type="number" min="0" value="${item.price}" data-walkin-item="price" data-walkin-index="${index}"></label><label>Discount<input type="number" min="0" value="${item.discount || 0}" data-walkin-item="discount" data-walkin-index="${index}"></label><div class="walkin-line-total"><small>Line total</small><strong data-walkin-line-total="${index}">${money(lineTotal)}</strong></div><button class="icon-btn" type="button" title="Remove service" data-remove-walkin-line="${index}">×</button></div>`;
}
