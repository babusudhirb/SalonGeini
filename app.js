...
function reports() {
  ...
  const reportOptions = Object.entries(definitions).map(([key, item]) => `<option value="${key}" ${key === reportView ? 'selected' : ''}>${item.label}</option>`).join('');
  layout(`${pageHead('BUSINESS INSIGHTS', report.title, report.description, `<label class="report-picker"><span>REPORT VIEW</span><select id="report-view-select">${reportOptions}</select></label>`)}<section class="report-kpi-grid">${report.metrics.map(([label, value, detail]) => `<article class="report-kpi-card"><span>${label}</span><strong>${value}</strong><small>${detail}</small></article>`).join('')}</section><section class="report-action-section"><div class="report-action-heading"><div><span class="eyebrow">WHAT TO DO NEXT</span><h2>Actionable insights</h2><p>Use these next steps to turn this report into progress.</p></div><span>${report.actions.length} priorities</span></div><div class="report-action-grid">${report.actions.map(([title, detail], index) => `<article><span>${index + 1}</span><div><h3>${title}</h3><p>${detail}</p><button class="text-link" data-report-action="${title}">Take action →</button></div></article>`).join('')}</div></section>`);
  document.getElementById('report-view-select')?.addEventListener('change', event => { reportView = event.target.value; render(); });
